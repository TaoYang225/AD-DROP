...
def parse_opt():
    '''hyper_parameters
    '''
    parser = argparse.ArgumentParser(description = 'Run AD-DROP on SST-2')
    parser.add_argument('--output_dir', type=str, default='model', help='directory to store model')
    parser.add_argument('--bert_path', type=str, default='roberta-base', help='path of pre-trained model BERT/RoBERTa')
    ...
    parser.add_argument('--model_path', type=str, default='model/finetuned_RoBERTa_AD.pth', help='path of the finetuned model to test')
...
if __name__ == '__main__':
    ...
    if args.option == 'train':
        ...
        with open('SST-2.tsv', 'w', newline='') as f:
            tsv_w = csv.writer(f, delimiter='\t')
            tsv_w.writerow(['index', 'prediction'])
            for i, lab in enumerate(preds_id):
                tsv_w.writerow([str(i), label_map[lab]])
        f.close()
