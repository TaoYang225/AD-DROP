...
from .addrop_utils import generate_mask_rev
...
class Trainer:
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        encoder_attn_mask = attention_mask.unsqueeze(1).unsqueeze(2).expand([-1, self.model.config.num_attention_heads, attention_mask.size(-1), -1]) # [b, h, s, s]
        encoder_attn_mask_list = [encoder_attn_mask for i in range(self.model.config.num_hidden_layers)]
        encoder_attn_mask_list_A = torch.stack(encoder_attn_mask_list, dim=0)

        # decoder_attn_mask = attention_mask.unsqueeze(1).unsqueeze(2).expand([-1, self.model.config.num_attention_heads, input['decoder_input_ids'].size(-1), -1]) # [b, h, t, s]
        # decoder_attn_mask_list = [decoder_attn_mask for i in range(self.model.config.decoder_layers)]
        # decoder_attn_mask_list_A = torch.stack(decoder_attn_mask_list, dim=0)

        inputs['addrop_encoder_mask'] = encoder_attn_mask_list_A
        # inputs['addrop_decoder_mask'] = decoder_attn_mask_list_A
        # first forward

        outputs = model(**inputs)
        # loss_ori = outputs["loss"]
        encoder_attentions = outputs['attentions']
        if self.args.do_mask and (int(self.state.epoch) % 2) != 0 and not self.control.should_evaluate:
        # if self.args.do_mask:
            p_label = outputs["logits"].argmax(dim=-1)  # (batch_size, sequence_length, config.vocab_size) --> (batch_size, sequence_length)
            loss_fct = torch.nn.CrossEntropyLoss()
            if attention_mask is not None:
                active_loss = attention_mask.view(-1) == 1
                active_logits = outputs["logits"].view(-1, self.model.config.num_labels)[active_loss]
                active_labels = p_label.view(-1)[active_loss]
                p_loss = loss_fct(active_logits, active_labels)
            else:
                p_loss = loss_fct(outputs["logits"].view(-1, self.model.config.num_labels), p_label.view(-1))


            for layer in range(self.model.config.num_hidden_layers):
                encoder_attentions[layer].retain_grad()

                (grad,) = torch.autograd.grad(p_loss, encoder_attentions[layer], retain_graph=True)  # 梯度归因
                grad_masks = generate_mask_rev(self.args, -grad, encoder_attn_mask, attention_mask, device=attention_mask.device, num_heads=self.model.config.num_attention_heads)
                encoder_attn_mask_list[layer] = grad_masks.long()
                encoder_attn_mask_list_B = torch.stack(encoder_attn_mask_list, dim=0)

            # 注入model中
            inputs['addrop_encoder_mask'] = encoder_attn_mask_list_B
            outputs = model(**inputs)
