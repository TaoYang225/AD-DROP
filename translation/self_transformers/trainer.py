...
class Trainer:
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        encoder_attn_mask_list = [encoder_attn_mask for i in range(self.model.config.encoder_layers)]
        ...
        #     print('addrop')
            ...
            masked_lm_loss = loss_fct(outputs["logits"].view(-1, self.model.config.vocab_size), p_label.view(-1))
            for layer in range(self.model.config.num_hidden_layers):
                encoder_attentions[layer].retain_grad()

                (grad,) = torch.autograd.grad(masked_lm_loss, encoder_attentions[layer], retain_graph=True)  # 梯度归因
