...
class MarianAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            attn_weights = attn_weights.view(bsz, self.num_heads, tgt_len, src_len) + attention_mask # encoder mask is hear
...
class MarianEncoder(MarianPreTrainedModel):
    ...
    def forward(
        ...
        if addrop_encoder_mask is not None:
            addrop_encoder_mask = 1.0 - addrop_encoder_mask
            addrop_encoder_mask = addrop_encoder_mask.masked_fill(addrop_encoder_mask.bool(), torch.finfo(attention_mask.dtype).min)
        for idx, encoder_layer in enumerate(self.layers):
            if output_hidden_states:
                encoder_states = encoder_states + (hidden_states,) # 残差
            ...
            if self.training and (dropout_probability < self.layerdrop):  # skip the layer
                ...
                if getattr(self.config, "gradient_checkpointing", False) and self.training:
                    ...
                    layer_outputs = encoder_layer(
                        hidden_states,
                        addrop_encoder_mask[idx] if addrop_encoder_mask is not None else attention_mask,
    ...
    def forward(
        ...
        if encoder_outputs is None:
            encoder_outputs = self.encoder(
                ...
                addrop_encoder_mask=addrop_encoder_mask,
            )
